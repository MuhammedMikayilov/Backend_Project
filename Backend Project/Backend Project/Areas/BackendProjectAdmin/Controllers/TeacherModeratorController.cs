...
namespace Backend_Project.Areas.BackendProjectAdmin.Controllers
{
    ...
    public class TeacherModeratorController : Controller
    {
        ...
        public TeacherModeratorController(UserManager<AppUser> userManager,
        ...
        public async Task<IActionResult> Create(RegisterVM register)
        {
            ...
            bool isExistUserName = _userManager.Users.Where(us => us.isDelete == false&&User.IsInRole("TeacherModerator")).Any(us => us.UserName == newUser.UserName);
            if (isExistUserName)
            {
            ...
            #region Check email
            bool isExistEmail = _userManager.Users.Where(us => us.isDelete == false && User.IsInRole("TeacherModerator")).Any(us => us.Email == newUser.Email);
            if (isExistEmail)
            {
            ...
            await _userManager.AddToRoleAsync(newUser, Roles.TeacherModerator.ToString());
        ...
        public async Task<IActionResult> UpdateUser(string id, AppUser userNewParam)
        {
            ...
            #region check username
            bool isExistUserName = _userManager.Users.Where(us => us.isDelete == false || User.IsInRole("TeacherModerator")).Any(us => us.UserName == userNewParam.UserName);
            AppUser hasUserName = _userManager.Users.Where(us => us.isDelete == false || User.IsInRole("TeacherModerator")).FirstOrDefault(us => us.Id == id);
            if (isExistUserName)
            {
            ...
            #endregion
            #region Check email
            bool isExistEmail = _userManager.Users.Where(us => us.isDelete == false || User.IsInRole("TeacherModerator")).Any(us => us.Email == userNewParam.Email);
            AppUser hasEmail = _userManager.Users.Where(us => us.isDelete == false || User.IsInRole("TeacherModerator")).FirstOrDefault(us => us.Id == id);
            if (isExistEmail)
            {
